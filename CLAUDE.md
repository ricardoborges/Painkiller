...
**Project entry: context vs dashboard.** `/projects/[id]` renders `ProjectContext` until some iteration session is `COMPLETED` (`ProjectSessionStore.hasCompleted`), then `ProjectDashboard` (published URLs, cost summary, task totals, all sessions, "Baixar .zip", "Nova sessão"), and the context moves to `/projects/[id]/context`. There is no session sidebar: the session is chosen with `SessionPicker` in the header, next to the session stepper, and both are hidden on the project-level views (Painel, Contexto, Custos) once the dashboard exists.
...
**Attachments in the chat.** Files dropped on the chat, pasted into the composer or picked with "Anexar" go to `POST /api/analysis/{sid}/attachments` (20 MB cap), which writes them into the repo at `.painkiller/uploads/{sid}/` (`UPLOADS_RELATIVE`, added to `.git/info/exclude` so no `git add -A` commits them). The message then carries their paths; `AnalysisOrchestrator.send` accepts only paths under that session's folder and appends a block headed `ATTACHMENTS_HEADER` telling the agent to open them with its file tools — no harness-specific multimodal input. [lib/attachments.ts](web/src/lib/attachments.ts) hides that block in the transcript and shows the file names as chips; the header text is duplicated there.
