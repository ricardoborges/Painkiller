...
`.env` is loaded with `override=True` at import time in [server.py](painkiller/api/server.py) — before FastAPI imports — so env changes require a server restart, and `.env` wins over the shell environment. It only holds **infrastructure** (images, DB URL, Gitea service account, internal URLs, ports, model defaults). The administrator, the session-signing key, the public URL, the host path of `./storage`, Google sign-in and Coolify live **only** in the database (see **Setup wizard and platform settings**); there is no env fallback for them.
...
- `PAINKILLER_GITEA_PASSWORD` stays in `.env`: compose and the Gitea service account need it before the API runs.
